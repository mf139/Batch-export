...
def generate_anonymized_id(identifier):
    """Generate a hashed anonymized ID from the original identifier."""
...
for plan_details in plan_list:
    ...
    anonymized_pt_id = generate_anonymized_id(pt_id.strip())
    anonymized_plan_name = generate_anonymized_id(plan_name.strip())
    ...
    with open(result_filename, 'a', newline='') as csvfile:
        writer = csv.writer(csvfile, dialect='excel')
        writer.writerow([anonymized_pt_id, anonymized_plan_name, result, ' '.join(messages)])